...
public interface JiraRepository {
	...
	List<ReleaseListModel> getUnmappedReleasesList();
	...
	List<ReleaseModel> getReleaseDetailsInAutoByProductId(int productId);

	List<FeaturesModel> getFeatureDetailsByReleaseId(int releaseId);

	String assigneeByAssigneeId(int assigned_To);

	String getJira_issue_typebyFeatureTypeId(int fk_feature_type_id);

	int releasestatusidBySprintStatus(String sprintStatus);

	ResponseDTO assignFullDetailsofSprinttoReleaseAsClosedSprint(int productId, long sprintId, String sprintName,
			int sprintExistanceByReleaseId, Date startDate, Date endDate, int fk_user_id, SprintModel sm);

	int releaseIdBySprintIdAsClosedStatus(long sprintId, SprintModel sm, int productId);

	int updateFeaturesStatusAsDone(FeaturesModel fm, int releaseIdBySprintId, int productId);

	int updatefeaturesasbacklogs(BackLogsModel bl, int productId);
}
